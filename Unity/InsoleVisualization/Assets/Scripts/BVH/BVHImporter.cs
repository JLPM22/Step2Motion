...
namespace BVH
{
    ...
    public class BVHImporter
    {
        ...
        {
            return Import(bvh.text, scale, onlyFirstFrame);
        }

        public BVHAnimation Import(string bvh, float scale = 1.0f, bool onlyFirstFrame = false)
        {
            List<AxisOrder> channels = new List<AxisOrder>();
            bool jointsHavePositions = false;
            BVHAnimation animation = new BVHAnimation();

            Stack<int> parentIndexStack = new Stack<int>();
            char[] whitespace = new char[] { ' ', '\t', '\r', '\n' };
            string[] words = bvh.Split(whitespace, System.StringSplitOptions.RemoveEmptyEntries);
            ...
            while (brackets > 0 && --it > 0)
            {
                ...
                jointsHavePositions = ReadChannels(channels, words, ref w);
            ...
            for (int i = 0; i < numberFrames; i++)
            {
                ...
                {
                    if (jointsHavePositions && (j - 1) % 3 == 0)
                    {
                        // Skip joint position channels
                        w += 3;
                    }
        ...
        private bool ReadChannels(List<AxisOrder> channels, string[] words, ref int w, bool root = false)
        {
            bool jointsHavePositions = false;
